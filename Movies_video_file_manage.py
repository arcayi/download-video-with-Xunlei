...
work_dir = os.getcwd()
print('\nwork_dir: ', work_dir,'\n')
def split2group(string,seperator,groups):
    i = string.lower().rfind(seperator)
    # print(f'[string.rfind(seperator)]@{i}')
    if( i>0 ):
        nameSplits=string[0:i]
        dirName = nameSplits.strip()
...
with os.scandir(work_dir) as it:
    for entry in it:
        ...
        for suf in include_suffix:
            if entry.name.lower().endswith(suf):
